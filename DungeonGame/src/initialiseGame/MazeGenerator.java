...
public class MazeGenerator {
	...
	private void readParameters() {
		...
		Document doc = fr.getDocument("/initialiseGame/MazeProperties.xml");
	...
	private List<TwoVector> addCellToRoom(List<TwoVector> room, TwoVector cellToAdd) {
		...
		List<TwoVector> neighbours = getNeighbours(cellToAdd);
	...
	private double getRoomEnergy(List<TwoVector> room) {
		double corners = Math.abs(idealNumberOfCorners - getCorners(room)) * cornerWeighting;
		double doors = Math.abs(idealNumberOfDoors - getSortedDoors(room).size()) * doorWeighting;
		double sa = getSurfaceArea(room) * surfaceAreaWeighting;		//in this case, surface area should always be minimised.
		return corners + doors + sa;
	}
	...
	private List<TwoVector> getDoors(List<TwoVector> room) {
	...
	private List<List<TwoVector>> getSortedDoors(List<TwoVector> room) {
		...
		List<TwoVector> doors = getDoors(room);
	...
	private void addRoomToMaze(List<TwoVector> room) {
		for (int i = 0; i < room.size(); i++) {
			TwoVector v = room.get(i);
			if (getBool(v) && !room.containsAll(getNeighbours(v))) {
				setBool(v, null);
			}
			else if (!getBool(v) && room.containsAll(getNeighbours(v))) {
				setBool(v, true);
			}
		}
	}
	...
	private char getWallChar(TwoVector pos) {
		...
		if (((pos.getX() > 0) && (!getBool(pos.minusX()))) || ((pos.getX() < (maze[0].length - 1)) && (!getBool(pos.plusX())))) {
			horizontalWall = true;
		}
